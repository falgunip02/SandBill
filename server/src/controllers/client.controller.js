...
const createClient = asyncHandler(async (req, res) => {
  const { clientName, assignedPerson } = req.body;
  
  // Validate required fields
  if (!clientName || !assignedPerson) {
    throw new ApiError(400, "Client name and assigned person are required");
  }
  ...
  try {
    ...
    const newClient = await Client.create({
      ...
      clientPhoto,
      clientAssigned: [{ 
        userId: assignedPerson // This should be a valid user ID from the dropdown
      }]
    });
